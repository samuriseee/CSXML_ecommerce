    ...
    public class NhapHangController : Controller
    {
        ...
        public ActionResult Create_chiTietNhap(ChiTietNhap chiTietNhap)
        {
            if (ModelState.IsValid)
            {
                ...
                xmlSP.Load(Server.MapPath("~/App_Data/SanPham.xml"));
                foreach (XmlElement e in xmlSP.GetElementsByTagName("SanPham"))
                {
                    if(e.GetAttribute("id") == chiTietNhap.maSP)
                    {
                        int sl = int.Parse(e.GetAttribute("soLuong"));
                        e.SetAttribute("soLuong", sl + chiTietNhap.soLuong + "");
                        xmlSP.Save(Server.MapPath("~/App_Data/SanPham.xml"));
                        UpdateToSQL(chiTietNhap.maSP);
                        break;
                    }
                }
